...
G_DEFINE_TYPE (MusividMusicRow, musivid_music_row, GTK_TYPE_LIST_BOX_ROW)
...
MusividMusicRow *
musivid_music_row_new (const char *path)
{
  return g_object_new (MUSIVID_TYPE_MUSIC_ROW,
                       "path", path,
                       NULL);
}

static void
musivid_music_row_finalize (GObject *object)
{
  MusividMusicRow *self = (MusividMusicRow *)object;
...
static void
musivid_music_row_add_tags (MusividMusicRow *self,
                            GstTagList      *tags)
{
...
on_bus_message (GstBus     *bus,
                ...
                gpointer    udata)
{
  MusividMusicRow *self = MUSIVID_MUSIC_ROW (udata);
  ...
  switch (GST_MESSAGE_TYPE (message))
    ...
    case GST_MESSAGE_ERROR:
      ...
      musivid_show_notification ("Pipeline for %s tags got error: %s", basename, error->message);
      // Fallthrough.
    ...
    case GST_MESSAGE_TAG:
      ...
      musivid_music_row_add_tags (self, tags);
      break;
    default:
      break;
    }
...
static void
musivid_music_row_update_tags (MusividMusicRow *self)
{
...
musivid_music_row_get_property (GObject    *object,
                                ...
                                GParamSpec *pspec)
{
  MusividMusicRow *self = MUSIVID_MUSIC_ROW (object);
...
static void
musivid_music_row_set_property (GObject      *object,
                                guint         prop_id,
                                const GValue *value,
                                GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_PATH:
      ...
      musivid_music_row_update_tags (self);
...
static void
musivid_music_row_class_init (MusividMusicRowClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->finalize = musivid_music_row_finalize;
  object_class->get_property = musivid_music_row_get_property;
  object_class->set_property = musivid_music_row_set_property;

  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);
  gtk_widget_class_set_template_from_resource (widget_class, "/com/refi64/Musivid/musivid-music-row.ui");
...
static void
musivid_music_row_init (MusividMusicRow *self)
{
...
const char *
musivid_music_row_get_path (MusividMusicRow *self)
{
  return self->path;
}

const char *
musivid_music_row_get_result_name (MusividMusicRow *self)
{
  return gtk_label_get_text (self->music_row_result_name);
}

void
musivid_music_row_toggle (MusividMusicRow *self)
{
