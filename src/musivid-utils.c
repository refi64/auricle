...
void
musivid_show_notification_internal (char *message)
{
  ...
  for (GList *l = windows; l != NULL; l = l->data)
    {
      if (MUSIVID_IS_WINDOW (l->data))
        {
          MusividWindow *win = MUSIVID_WINDOW (l->data);
          musivid_window_show_notification (win, message);
          goto done;
        }
    }

  g_warning ("musivid_show_notification failed to find main window");
...
char *
musivid_substitute (const char *template,
                    GHashTable *vars)
{
